...
Shader "Universal Render Pipeline/Custom/MartianNoisyGlow"
{
    Properties
    {
        ...
        [MainTexture] _NoiseTex("Noise Tex", 2D) = "white" {}
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 LitPassFragment(Varyings input) : SV_Target
            {
                ...
                float2 noiseUV = input.positionWSAndFogFactor.xz * 0.01f + float2(.5, .5);
                float noise = .1 + .9*SAMPLE_TEXTURE2D(_NoiseTex, sampler_LinearClamp, noiseUV).r;
                return half4(color * noise * 1.5, surfaceData.alpha);
            }
            ENDHLSL
        }
